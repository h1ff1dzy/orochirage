...
const ImageUploader: React.FC<ImageUploaderProps> = ({ token, imageLink, itemId }) => {
    ...
    const handleImageUpload = async (image: File) => {
        try {
            ...
            await axios.post(
                `https://api.orochirage.ru/api/admin/collection/carousel/${itemId}/image/upload`,
                formData,
                {
                    headers: {
    ...
    const handleImageClick = () => {
        if (!selectedImage) {
            document.getElementById('carouselImg')?.click();
        }
    };
    ...
    return (
        <div className="flex flex-col items-center" key="carousel_ipload">
            <input type="file" accept="image/png, image/jpeg, image/jpg" id="carouselImg" onChange={handleImageChange} style={{ display: 'none' }} />
